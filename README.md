...
- **行业标识**：租户带 `industry` 字段，取值 GB/T 4754-2017 国民经济行业分类码（门类/大类/中类/小类四级，全量 1971 条入 `industries` 集合，启动自动种子）。插件经 `Industries()` 声明适用行业码——租户行业码的祖先链含声明码即适用（如插件声明大类 `80` 居民服务业，理发店 `8040` 可用）；行业插件只对匹配租户出现在插件列表，留空插件为通用。启用时服务端同样校验适用性。
