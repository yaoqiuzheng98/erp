...
func New(e *env.Env) *Service {
	return &Service{e: e, repo: repo.NewTenantRepo[model.Order](e.DB.Database, "plg_sales_order")}
}
...
func (s *Service) Create(ctx context.Context, tenantID bson.ObjectID, o *model.Order, by string) error {
	...
	no, err := s.e.Seq.Next(ctx, tenantID, "SO")
	if err != nil {
		return err
	}
	var total float64
...
// Submit 提交：flow 插件启用则走审批流，否则直接审核通过。
	...
	if s.e.Gate.IsEnabled(ctx, tenantID, "flow") {
		...
		s.e.Events.Publish(ctx, event.Event{
			...
			Payload: contract.DocSubmit{
				TargetType: "sales_order", TargetID: id.Hex(),
				Title: "销售订单 " + o.DocNo, By: by,
			},
		})
		return nil
	}
...
func (s *Service) approve(ctx context.Context, tenantID bson.ObjectID, o *model.Order, by string) error {
	...
	s.e.Events.Publish(ctx, event.Event{
		Topic:    contract.TopicSalesApproved,
		TenantID: tenantID,
		Payload: contract.OrderApproved{
			OrderID: o.ID.Hex(), DocNo: o.DocNo,
			PartnerID: o.CustomerID, Total: o.Total,
			Lines: o.Lines, By: by,
		},
	})
	return nil
...
// OnFlowResult 审批流结论回写（flow.approved / flow.rejected 事件）。
